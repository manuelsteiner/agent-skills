...
An exact candidate has matching track order, audible-track count, disc assignment, and edition structure. Prefer it over a higher-scoring candidate that is only plausible. When several candidates are equally exact, prefer the original-era release. In the least-restrictive completion path, this is an approved tie-breaker when no local catalog, barcode, label, country, medium, duration, hidden-track, or bonus-track evidence points to a different pressing. If that evidence conflicts, the candidates are not equivalent. Defer them in `auto-safe` and pause in ordinary `guided` work unless the user has granted this tie-breaker as standing authority.
